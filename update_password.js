...
function xorCipher(text, pass) {
    ...
    // Convert to Base64 to make it safe for HTML/JS string
    return Buffer.from(result).toString('base64');
}

// Encrypt
const encryptedLetter = xorCipher(letterContent, password);
const encryptedMessage = xorCipher(message, password);
...
fs.writeFileSync('encrypted_payload_new.json', JSON.stringify(payload, null, 2));
console.log("Encrypted payload written to encrypted_payload_new.json");
